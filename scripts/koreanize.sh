...
# 4) 빌드된 docs/index.html은 'index' 청크가 그대로 만든 산점도 데모입니다.
# 루트 경로(/ → showcase) 리다이렉트는 public/index.html 템플릿에 주입된
# 인라인 <script>가 pathname==='/'일 때만 실행하므로, JS 환경이면 자동으로
# 쇼케이스로 이동하고 JS 미지원 환경에서는 단일 데모가 그대로 보인다.
# 따라서 koreanize.sh는 docs/index.html을 덮어쓰지 않는다.

# 5) menu-*.js 자동 한글화
MENU_JS=$(ls docs/assets/menu-*.js 2>/dev/null | head -1)
if [ -n "$MENU_JS" ]; then
  cat > /tmp/ulw-koreanize-replacements.json <<'JSON'
[
  ["Details", "상세"],
  ["Settings", "설정값"],
  ["Examples", "예제 목록"],
...
JSON
  MENU_JS="$MENU_JS" python3 - <<'PYEOF'
import json, os, re
from pathlib import Path

menu_path = os.environ["MENU_JS"]
content = Path(menu_path).read_text()
replacements = json.loads(Path("/tmp/ulw-koreanize-replacements.json").read_text())

# 라소 종류 옵션(Freeform/Brush/Rectangle)이 라소 옵션 키로만 적용되도록
# 식별자 키 뒤에 콜론과 백틱이 오는 패턴만 치환한다.
LASSO_TYPE_KEYS = {"Freeform", "Brush", "Rectangle"}

total = 0
replacements_sorted = sorted(replacements, key=lambda r: -len(r[0]))
for old, new in replacements_sorted:
    if old in LASSO_TYPE_KEYS:
        pattern = re.compile(rf"\b{re.escape(old)}:(?=\`[a-z]+\`)")
...
PYEOF
  rm -f /tmp/ulw-koreanize-replacements.json
fi
